...
void forward(){
  ...
  analogWrite(LEFT_PWM, 150);//158 180
  //右輪·因在小車上馬達安装方向左右兩個是相
  digitalWrite(RIGHT1, LOW);
  digitalWrite (RIGHT2, HIGH);
  analogWrite(RIGHT_PWM, 160);//190 
}
void backward(){
  digitalWrite(LEFT1, HIGH);
  digitalWrite(LEFT2, LOW);
...
void loop() {
  ...
  bool mid = digitalRead(5);
  if (rightSensor && leftSensor && mid) {//
    stopMotor();
    delay(5);
    ...
    else if(!rightSensor && !leftSensor && !mid){
     forward();
     delay(5);
   }
   stopMotor();
